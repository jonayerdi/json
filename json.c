...
/* Parsing utils */
static int
json_is_whitespace(json_char character)
{
...
static json_char
json_next_token(const json_char* json, json_size length, json_size* read)
{
    json_size index = 0;
    do {
        if (index == length) {
...
/* Read json elements */
json_state
json_read_value(const json_char* json,
    json_size length,
    json_value* data_out,
    json_size* read)
{
    ...
    case '\"':
        ...
        sub_retval = json_read_string(
            json + index, length - index, &data_out->value_str, &sub_read);
    ...
    case '{':
        ...
        sub_retval = json_read_object(
            json + index, length - index, &data_out->value_obj, &sub_read);
        if (sub_retval != json_state_ok) {
            return sub_retval;
        }
        *read = index + sub_read;
    ...
    case '[':
        ...
        sub_retval = json_read_array(
            json + index, length - index, &data_out->value_array, &sub_read);
        if (sub_retval != json_state_ok) {
            return sub_retval;
        }
        *read = index + sub_read;
    ...
    default:
        // Number
        sub_retval = json_read_integer(
            json + index, length - index, &data_out->value_int, &sub_read);
        if (sub_retval != json_state_ok) {
            return sub_retval;
        }
        if (json[index + sub_read] == JSON_DECIMAL_COMMA[0]) // Decimal
        {
            ...
            sub_retval = json_read_decimal(
                json + index, length - index, &data_out->value_dec, &sub_read);
            if (sub_retval != json_state_ok) {
                return sub_retval;
            }
        } else
...
json_state
json_read_string(const json_char* json,
    json_size length,
    json_char** data_out,
    json_size* read)
{
...
json_state
json_read_integer(const json_char* json,
    json_size length,
    json_integer* data_out,
    json_size* read)
{
    json_size index = 0;
    int digit;
...
json_state
json_read_decimal(const json_char* json,
    json_size length,
    json_decimal* data_out,
    json_size* read)
{
    json_size index = 0;
...
json_state
json_read_key_value(const json_char* json,
    json_size length,
    json_key_value* data_out,
    json_size* read)
{
    json_size index = 0;
    json_size sub_read;
...
json_state
json_read_object(const json_char* json,
    json_size length,
    ...
    json_size* read)
{
    ...
    while (json_next_token(json + index, length - index, &sub_read) != JSON_OBJECT_CLOSE[0]) {
        sub_retval = json_read_key_value(
            json + index, length - index, buffer + element_count, &sub_read);
        if (sub_retval != json_state_ok) {
...
json_state
json_read_array(const json_char* json,
    json_size length,
    json_array* data_out,
    json_size* read)
{
...
json_value*
json_object_find_key(const json_object* object,
    const json_char* key,
    json_size num)
{
...
{
    if (value == NULL || value->type != json_type_integer) {
        return NULL;
    }
...
{
    if (value == NULL || value->type != json_type_decimal) {
        return NULL;
    }
